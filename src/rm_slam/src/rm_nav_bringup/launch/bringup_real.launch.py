...
def generate_launch_description():
    ...
    pointlio_rviz_cfg_dir = os.path.join(rm_nav_bringup_dir, 'rviz', 'loam_livox.rviz')
    ...
    declare_localization_cmd = DeclareLaunchArgument(
        'localization',
        default_value='',
        description='Choose localization method: slam_toolbox, amcl, icp, small_gicp')
    ...
    # 用 TimerAction 包装，延迟 5 秒启动
    delayed_state_publisher = TimerAction(
        period=3.0,  # 5 秒延迟
    ...
    # 用 TimerAction 包装，延迟 5 秒启动
    delayed_livox_driver = TimerAction(
        period=3.0,  # 5 秒延迟
    ...
    delayed_imu_filter = TimerAction(
        period=3.0,  # 5 秒延迟
        actions=[bringup_imu_complementary_filter_node]
    )
    ...
    ld.add_action(delayed_state_publisher)
    ld.add_action(delayed_livox_driver)
    ld.add_action(delayed_imu_filter)
